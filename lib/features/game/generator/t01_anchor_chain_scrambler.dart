...
class T01AnchorChainScrambler {
  const T01AnchorChainScrambler({
  ...
  bool supports({
    ...
    if (!_hasValidPlacements(targetPlacements)) {
      return false;
    }
    if (spec.targetSwapCount < 1 ||
        spec.targetSwapCount > spec.totalBookCount - 1) {
      return false;
    }
    if (clues.isEmpty || clues.length != spec.clueCount) {
      return false;
    }
    if (!_hasUniqueClueIds(clues)) {
      return false;
    }
  ...
  TemplateScrambleResult create({
    ...
    if (!supports(solution: solution, clues: clues)) {
      throw StateError('T01 Anchor Chain cannot scramble this input.');
    }

    final sortedTargets = _sortedPlacements(solution.targetPlacements);
    _validateTargetClues(clues: clues, targetPlacements: sortedTargets);
    ...
    throw StateError('T01 Anchor Chain could not find a valid scramble.');
  }
  ...
  bool _isValidCandidate({
    ...
    if (permutationAnalyzer.minimumSwapDistance(
  ...
  void _validateResult({
    ...
    if (result.scrambleSeed < 0 ||
        result.scrambleSeed > GeneratorConfig.uint32Mask) {
      throw StateError('T01 scramble seed is outside uint32 range.');
    }
    if (result.initialPlacements.length != sortedTargets.length) {
      throw StateError('T01 initial placement count does not match target.');
    }
    if (permutationAnalyzer.hasSameBookOrder(
      first: result.initialPlacements,
      second: sortedTargets,
    )) {
      throw StateError('T01 initial placements match target placements.');
    }
    if (result.swapHistory.length != result.targetSwapCount) {
      throw StateError(
        'T01 swap history length does not match targetSwapCount.',
      );
    }
    if (!_hasValidPlacements(result.initialPlacements)) {
      throw StateError('T01 initial placements are invalid.');
    }
    if (!_sameBookIdSet(sortedTargets, result.initialPlacements)) {
      throw StateError('T01 initial book ids do not match target ids.');
    }
    if (!_sameVisualSet(sortedTargets, result.initialPlacements)) {
      throw StateError('T01 initial visual books do not match target.');
    }
    if (_allCluesSatisfied(
      clues: clues,
      placements: result.initialPlacements,
    )) {
      throw StateError('T01 initial placements already satisfy every clue.');
    }
    final minimumSwapDistance = permutationAnalyzer.minimumSwapDistance(
      target: sortedTargets,
      current: result.initialPlacements,
    );
    if (minimumSwapDistance != result.targetSwapCount) {
      throw StateError(
        'T01 minimum swap distance does not match targetSwapCount.',
      );
    }
    if (!permutationAnalyzer.hasSameBookOrder(
      ...
      throw StateError('T01 forward replay does not match initial placements.');
    }
    if (!permutationAnalyzer.hasSameBookOrder(
      ...
      throw StateError('T01 reverse replay does not match target placements.');
    }
  }

  void _validateTargetClues({
    required List<Clue> clues,
    required List<BookPlacement> targetPlacements,
  }) {
    ...
    if (satisfiedIds.length != clues.length) {
      throw StateError('T01 target placements do not satisfy every clue.');
    }
    for (final clue in clues) {
      if (!satisfiedIds.contains(clue.id)) {
        throw StateError('T01 target placements miss clue: ${clue.id}');
      }
    }
  }
  ...
  bool _sameVisualSet(List<BookPlacement> target, List<BookPlacement> initial) {
    return _setEquals(_visualSet(target), _visualSet(initial));
  }
  ...
  Set<String> _visualSet(List<BookPlacement> placements) {
    return {for (final placement in placements) _visualKey(placement.book)};
  }

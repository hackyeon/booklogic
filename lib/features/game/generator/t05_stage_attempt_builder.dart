...
import 't05_tier_order_solution_factory.dart';

class T05StageAttemptBuilder {
  const T05StageAttemptBuilder({
    this.solutionFactory = const T05TierOrderSolutionFactory(),
  ...
  final T05TierOrderSolutionFactory solutionFactory;
  ...
  GeneratedStage build({
    ...
    if (!supports(stageSpec)) {
      throw UnsupportedError('T05 attempt builder does not support StageSpec.');
    }

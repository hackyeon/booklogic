...
class T03AdjacentBlocksScrambler {
  const T03AdjacentBlocksScrambler({
  ...
  bool supports({
    ...
    if (!_hasT03TargetStructure(targetPlacements)) {
      return false;
    }
  ...
  TemplateScrambleResult create({
    ...
    if (!supports(solution: solution, clues: clues)) {
      throw StateError('T03 Adjacent Blocks cannot scramble this input.');
    }
    ...
    throw StateError('T03 Adjacent Blocks could not find a valid scramble.');
  }

  int createScrambleSeed(int stageSeed) {
    final value =
        (stageSeed ^ GeneratorConfig.t03ScrambleSalt) &
  ...
  void _validateResult({
    ...
    if (result.scrambleSeed < 1 ||
      ...
      throw StateError('T03 scramble seed is outside uint32 range.');
    }
    if (result.initialPlacements.length != sortedTargets.length) {
      throw StateError('T03 initial placement count does not match target.');
    }
    if (permutationAnalyzer.hasSameVisualOrder(
      first: result.initialPlacements,
      second: sortedTargets,
    )) {
      throw StateError('T03 initial visual order matches target.');
    }
    if (result.swapHistory.length != result.targetSwapCount) {
      throw StateError(
        'T03 swap history length does not match targetSwapCount.',
      );
    }
    if (!_hasValidPlacements(result.initialPlacements)) {
      throw StateError('T03 initial placements are invalid.');
    }
    if (!_sameBookIdSet(sortedTargets, result.initialPlacements)) {
      throw StateError('T03 initial book ids do not match target ids.');
    }
    if (!_sameVisualMultiset(sortedTargets, result.initialPlacements)) {
      throw StateError('T03 initial visual books do not match target.');
    }
    if (_allCluesSatisfied(
      clues: clues,
      placements: result.initialPlacements,
    )) {
      throw StateError('T03 initial placements already satisfy every clue.');
    }
    ...
    if (minimumSwapDistance != result.targetSwapCount) {
      throw StateError(
        'T03 visual minimum swap distance does not match targetSwapCount.',
      );
    }
    if (!permutationAnalyzer.hasSameBookOrder(
      ...
      throw StateError('T03 forward replay does not match initial placements.');
    }
    if (!permutationAnalyzer.hasSameBookOrder(
      ...
      throw StateError('T03 reverse replay does not match target placements.');
    }
  }
  ...
  bool _hasT03TargetStructure(List<BookPlacement> placements) {
    final sorted = _sortedPlacements(placements);
    if (!_hasSingleDuplicatePair(sorted)) {
      return false;
    }
    if (_visualKey(sorted[3].book) != _visualKey(sorted[4].book)) {
      return false;
    }
    final colors = {
      sorted[0].book.color,
  ...
  bool _hasSingleDuplicatePair(List<BookPlacement> placements) {
